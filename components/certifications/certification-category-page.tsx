...
import { ArrowRight, BadgeCheck, Layers3, Route, ShieldCheck } from "lucide-react";
...
export async function CertificationCategoryPage({
  ...
  if (error) {
    throw new Error(`Unable to load ${category.name} certifications.`);
  }

  const certifications = (data ?? []) as CategoryCertification[];
  const levels = [...new Set(certifications.map((item) => item.level))];
  const targetRoles = [
    ...new Set(certifications.flatMap((item) => item.target_job_roles ?? [])),
  ];
  const providers = [
    ...new Set(certifications.map((item) => providerName(item.providers))),
  ];
  ...
  return (
    <main>
      ...
      <MarketingHero
        aside={
          <HeroPanel>
            <h2 className="text-xl font-bold text-white">Category snapshot</h2>
            <p className="mt-1 text-sm text-slate-300">
              A quick view of the credentials currently in Badgely.
            </p>
            ...
            <div className="mt-5 space-y-3">
              <HeroOption className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-white">{certifications.length} certifications</p>
                  <p className="mt-1 text-sm text-slate-300">Active records in this category.</p>
                </div>
              </HeroOption>
              ...
              <HeroOption className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-white">{levels.length} levels represented</p>
                  <p className="mt-1 text-sm text-slate-300">{levels.join(" · ")}</p>
                </div>
              </HeroOption>
              ...
              <HeroOption className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-white">{targetRoles.length} target roles</p>
                  <p className="mt-1 text-sm text-slate-300">
      ...
      >
        ...
        <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">{category.intro}</p>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl leading-7 text-slate-600">
              Review the level, provider, career fit, and verified details for each certification before choosing your next step.
            </p>
          </div>
          ...
          {certifications.map((certification) => (
            <Card key={certification.id} className="flex h-full flex-col">
              <div className="flex flex-wrap items-center gap-2">
                <Badge>{providerName(certification.providers)}</Badge>
                <Badge>{certification.level}</Badge>
                <Badge>{certification.vendor_type}</Badge>
              </div>

              <h3 className="mt-5 text-xl font-bold text-slate-950">{certification.name}</h3>
              ...
              {certification.target_job_roles?.length ? (
                <div className="mt-4 flex flex-wrap gap-2">
                  {certification.target_job_roles.slice(0, 3).map((role) => (
                    <span
                      key={role}
              ...
              <div className="mt-6 flex items-center justify-between gap-4 border-t border-slate-200 pt-4">
                <span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-500">
                  ...
                  Verified {certification.last_verified_date ?? "date pending"}
                </span>
                <Link
