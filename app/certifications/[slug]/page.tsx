...
async function getCertification(slug: string) {
  ...
  const [
    ...
    supabase
      .from("exam_domains")
      .select(
        "id, domain_name, domain_weight_text, description, display_order",
      )
      .eq("certification_id", certification.id)
      .order("display_order"),
  ...
  const targetIds = [
    ...new Set(
      relations.map((relation) => relation.target_certification_id),
    ),
  ];
...
export default async function CertificationDetailPage({
  ...
  const relatedByType = {
    ...
    Alternative: relations.filter(
      (relation) => relation.relation_type === "Alternative",
    ),
    Specialization: relations.filter(
      (relation) => relation.relation_type === "Specialization",
    ),
  };
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="mt-6 rounded-3xl bg-slate-950 px-6 py-10 text-white sm:px-10">
        ...
        <p className="mt-6 text-sm text-slate-400">
          Last verified:{" "}
          {certification.last_verified_date ?? VERIFY_TEXT}
        </p>
      </header>
              ...
              <dl className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <dd className="mt-1 text-slate-900">
                    {displayValue(
                      certification.recommended_experience,
                    )}
                  </dd>
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-2 flex flex-wrap gap-2">
                    {certification.target_job_roles.length ? (
                      certification.target_job_roles.map((role) => (
                  ...
                  <Card key={exam.id}>
                    ...
                    <dl className="mt-4 grid gap-4 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <dd>
                          {displayValue(exam.question_count_text)}
                        </dd>
                      </div>
                      ...
                      <div>
                        ...
                        <dd>
                          {displayValue(exam.delivery_method)}
                        </dd>
                      </div>
                  ...
                  <Card key={resource.id}>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {resource.is_official ? (
                        <Badge>Official resource</Badge>
                      ) : null}
                    </div>

                    <p className="mt-2 text-sm text-slate-500">
                      {resource.resource_type} ·{" "}
                      {displayValue(resource.cost_type)}
                    </p>
            ...
            <Card className="mt-4">
              <dl className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <dd>
                    {displayValue(renewal?.validity_period_text)}
                  </dd>
                </div>
                    ...
                    <ul className="mt-3 space-y-2">
                      {group.map((relation) => {
                        const related = relatedName(
                          relation.target_certification_id,
                        );

                        return (
                          <li
                            key={`${relation.relation_type}-${relation.target_certification_id}`}
                          >
                            {related ? (
                              <Link
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <Card>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Information may change. Confirm current exam details,
              pricing, availability, and renewal requirements with the
              official provider before making decisions.
            </p>
          </Card>
