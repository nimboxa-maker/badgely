...
async function getCertification(slug: string) {
  ...
  const [providerResult, examsResult, domainsResult, resourcesResult, renewalResult, relationsResult] =
    await Promise.all([
      supabase.from("providers").select("name, slug").eq("id", certification.provider_id).maybeSingle(),
      supabase
        .from("exams")
        .select(
      ...
      supabase
        ...
        .select("validity_period_text, renewal_method, official_renewal_url, notes, last_verified_date")
        .eq("certification_id", certification.id)
        .maybeSingle(),
      supabase
...
export default async function CertificationDetailPage({ params }: CertificationPageProps) {
  ...
  const { certification, provider, exams, domains, resources, renewal, relations, relatedCertifications } =
    record;

  const studyHours =
    certification.estimated_study_hours_min !== null && certification.estimated_study_hours_max !== null
  ...
  const relatedByType = {
    "Recommended Before": relations.filter((relation) => relation.relation_type === "Recommended Before"),
    "Recommended After": relations.filter((relation) => relation.relation_type === "Recommended After"),
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <Link href="/certifications" className="text-sm font-semibold text-blue-700 hover:text-blue-600">
      ...
      <header className="mt-6 rounded-3xl bg-slate-950 px-6 py-10 text-white sm:px-10">
        ...
        <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">{certification.short_summary}</p>

        <div className="mt-8 flex flex-wrap gap-3">
          <button
            type="button"
            disabled
            title="Certification saving will be enabled in Milestone 4."
            className="min-h-11 rounded-xl bg-blue-600 px-4 py-2.5 font-semibold text-white opacity-60"
          >
            Save certification
          </button>
          <button
            ...
            <Card className="mt-4 space-y-5">
              ...
              <dl className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <dd className="mt-1 text-slate-900">{displayValue(certification.recommended_experience)}</dd>
                </div>
                <div>
                ...
                exams.map((exam) => (
                  <Card key={exam.id}>
                    ...
                    <dl className="mt-4 grid gap-4 sm:grid-cols-2">
                      <div><dt className="text-sm font-semibold text-slate-500">Exam code</dt><dd>{displayValue(exam.exam_code)}</dd></div>
                      <div><dt className="text-sm font-semibold text-slate-500">Number of exams</dt><dd>{displayValue(exam.number_of_exams)}</dd></div>
                      <div><dt className="text-sm font-semibold text-slate-500">Duration</dt><dd>{exam.duration_minutes ? `${exam.duration_minutes} minutes` : VERIFY_TEXT}</dd></div>
                      <div><dt className="text-sm font-semibold text-slate-500">Question count</dt><dd>{displayValue(exam.question_count_text)}</dd></div>
                      <div><dt className="text-sm font-semibold text-slate-500">Delivery method</dt><dd>{displayValue(exam.delivery_method)}</dd></div>
                      <div><dt className="text-sm font-semibold text-slate-500">Price</dt><dd>{displayValue(exam.price_text)}</dd></div>
                    </dl>
                  </Card>
                ))
              ) : (
                <Card><p className="text-slate-700">{VERIFY_TEXT}</p></Card>
              )}
            </div>
          </section>
                ...
                domains.map((domain) => (
                  <Card key={domain.id}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <span className="text-sm text-slate-500">{displayValue(domain.domain_weight_text)}</span>
                    </div>
                    <p className="mt-2 leading-7 text-slate-600">{displayValue(domain.description)}</p>
                  </Card>
                ))
              ) : (
                <Card><p className="text-slate-700">{VERIFY_TEXT}</p></Card>
              ...
              {resources.length ? (
                resources.map((resource) => (
                  <Card key={resource.id}>
                    ...
                    <p className="mt-3 leading-7 text-slate-600">{displayValue(resource.description)}</p>
                    {resource.url ? (
                      <a
                        href={resource.url}
                        target="_blank"
          ...
          <section aria-labelledby="renewal-heading">
            ...
            <Card className="mt-4">
              <dl className="grid gap-4 sm:grid-cols-2">
                <div><dt className="text-sm font-semibold text-slate-500">Validity period</dt><dd>{displayValue(renewal?.validity_period_text)}</dd></div>
                <div><dt className="text-sm font-semibold text-slate-500">Renewal method</dt><dd>{displayValue(renewal?.renewal_method)}</dd></div>
                <div className="sm:col-span-2"><dt className="text-sm font-semibold text-slate-500">Notes</dt><dd>{displayValue(renewal?.notes)}</dd></div>
              </dl>
            </Card>
          </section>
          ...
          <section aria-labelledby="related-heading">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {Object.entries(relatedByType).map(([label, group]) => (
                <Card key={label}>
                  <h3 className="font-semibold">{label === "Alternative" ? "Alternatives" : `${label}${label === "Specialization" ? "s" : ""}`}</h3>
                  {group.length ? (
                      ...
                      {group.map((relation) => {
                        ...
                        return (
                          <li key={`${relation.relation_type}-${relation.target_certification_id}`}>
                            {related ? (
                              <Link href={`/certifications/${related.slug}`} className="font-semibold text-blue-700 hover:text-blue-600">
                                {related.name}
                              </Link>
                            ) : (
                              VERIFY_TEXT
                            )}
                            {relation.explanation ? <p className="mt-1 text-sm text-slate-600">{relation.explanation}</p> : null}
                          </li>
                        );
                      })}
                    </ul>
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <Card>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Information may change. Confirm current exam details, pricing, availability, and renewal requirements with the official provider before making decisions.
            </p>
          </Card>
          <Card>
